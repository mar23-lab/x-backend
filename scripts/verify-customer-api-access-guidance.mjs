...
check('environment_bound_gateway_endpoint', /buildConnectorEndpoints\(new URL\(ctx\.req\.url\)\.origin\)/.test(source.route) && /XCP_GATEWAY_RPC_PATH/.test(source.route) && /XCP_GATEWAY_SESSION_START_PATH/.test(source.route), 'Connector discovery and token mint receipts bind to the authenticated API environment.');
check('no_production_endpoint_literal', !/https:\/\/api\.xlooop\.com\/api\/v1\/mcp\//.test(source.route), 'Developer-access responses contain no hardcoded production connector endpoint.');
check('environment_endpoint_regression_test', /api-test\.xlooop\.com\/api\/v1\/mcp\/rpc/.test(source.tests) && /api-test\.xlooop\.com\/api\/v1\/mcp\/session-start/.test(source.tests) && /not\.toContain\('https:\/\/api\.xlooop\.com'\)/.test(source.tests), 'Route regressions prove status, test, and mint responses preserve the non-production API origin.');
