...
check('environment_bound_gateway_endpoint', /new URL\(ctx\.req\.url\)\.origin/.test(source.route) && /XCP_GATEWAY_RPC_PATH/.test(source.route) && /XCP_GATEWAY_SESSION_START_PATH/.test(source.route), 'Connector discovery binds to the authenticated API environment instead of hardcoding production.');
check('environment_endpoint_regression_test', /api-test\.xlooop\.com\/api\/v1\/mcp\/rpc/.test(source.tests) && /api-test\.xlooop\.com\/api\/v1\/mcp\/session-start/.test(source.tests), 'Route regression proves non-production deployments advertise their own gateway origin.');
