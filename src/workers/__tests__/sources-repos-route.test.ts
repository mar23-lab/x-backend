...
describe('GET /sources/:id/folders · workspace guard', () => {
  it('404 before provider access when the owned source belongs to another workspace', async () => {
    const res = await appFor(
      { user_id: 'u1', workspace_id: 'ws1' },
      async () => ({ id: 's1', provider: 'google_drive', workspace_id: 'ws2' }),
    ).request('/api/v1/sources/s1/folders', {}, ENV as never);
    expect(res.status).toBe(404);
  });
});
