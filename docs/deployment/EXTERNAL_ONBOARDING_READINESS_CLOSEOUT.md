...
| API/MCP live canary | `XLOOOP_PARITY_PACKET_ID=pkt-canary-*`, `XLOOOP_CANARY_API_TOKEN_FILE`, `XLOOOP_CANARY_LIFECYCLE_API_TOKEN_FILE` | `npm run verify:api-mcp-lifecycle-parity -- --format=json`; `npm run verify:api-mcp-live-canary-hard-stop -- --strict-live --target=production` | `api_mcp_live_canary_authority=true` | Sanitized canary result summary | `whoami` binds correct tenant/user; read-only packet works; lifecycle canary proves revocation blocks access; declared target matches API base | Revoke canary tokens; remove/expire canary packet; keep customer MCP read-only |
